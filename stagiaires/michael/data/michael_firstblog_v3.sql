...
-- -----------------------------------------------------
-- Table `michael_firstblog`.`article`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `michael_firstblog`.`article` ;

CREATE TABLE IF NOT EXISTS `michael_firstblog`.`article` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `title` VARCHAR(90) NOT NULL,
  `content` TEXT NOT NULL,
  `datetime` DATETIME NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'ou now(), temps actuel si non renseigné',
  `actif` TINYINT NULL DEFAULT 0 COMMENT '0 => non visible\n1 => visible\n2 =>  article caché',
  ...
  CONSTRAINT `fk_article_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `michael_firstblog`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `michael_firstblog`.`category`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `michael_firstblog`.`category` ;

CREATE TABLE IF NOT EXISTS `michael_firstblog`.`category` (
...
-- -----------------------------------------------------
-- Table `michael_firstblog`.`category_has_article`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `michael_firstblog`.`category_has_article` ;

CREATE TABLE IF NOT EXISTS `michael_firstblog`.`category_has_article` (
  ...
  CONSTRAINT `fk_category_has_article_category1`
    ...
    REFERENCES `michael_firstblog`.`category` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_category_has_article_article1`
    FOREIGN KEY (`article_id`)
    REFERENCES `michael_firstblog`.`article` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
